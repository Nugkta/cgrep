#!/bin/bash --login
#SBATCH --job-name=mibig1_eval
#SBATCH --output=logs/mibig1_evaluation/mibig1_eval_%j.out
#SBATCH --error=logs/mibig1_evaluation/mibig1_eval_%j.err
...
mkdir -p logs/mibig1_evaluation
...
COMMAND="python scripts/classification/mibig1_stratified_evaluation.py \
  --artifacts_dir artifacts/classification/mibig1 \
  --outdir results/mibig1_classification \
  --seed 42"
...
echo "Available MIBiG 1.0 embedding files:"
ls -la artifacts/classification/mibig1/
echo ""

# Run evaluation
echo "Running command: $COMMAND"
echo "Evaluation started at: $(date)"
echo "Note: Results will be saved under results/mibig1_classification/"
echo ""
...
if [ $EVAL_EXIT_CODE -eq 0 ]; then
    ...
    if [ -f "results/mibig1_classification/mibig1_comparison.csv" ]; then
        echo "Model comparison table:"
        cat results/mibig1_classification/mibig1_comparison.csv
    fi
else
...
echo "Results saved in $WORK_DIR/results/mibig1_classification/"
