#!/bin/bash --login
#SBATCH --job-name=mibig1_bootstrap
#SBATCH --output=logs/mibig1_evaluation/mibig1_bootstrap_%j.out
#SBATCH --error=logs/mibig1_evaluation/mibig1_bootstrap_%j.err
...
echo "🚀 MIBiG 1.0 Bootstrap Evaluation Started"
...
if [ $EVAL_EXIT_CODE -eq 0 ]; then
    ...
    if [ -f "results/mibig1_bootstrap_evaluation/bootstrap_analysis/mibig1_bootstrap_summary.csv" ]; then
        echo ""
        echo "📈 Performance Summary (Macro AUC Focus):"
        echo "Model,Macro_AUC_Mean,Macro_AUC_Std,95%_CI_Lower,95%_CI_Upper"
        tail -n +2 results/mibig1_bootstrap_evaluation/bootstrap_analysis/mibig1_bootstrap_summary.csv | \
...
echo "📁 Results: $WORK_DIR/results/mibig1_bootstrap_evaluation/"
